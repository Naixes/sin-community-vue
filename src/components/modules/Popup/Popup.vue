...
<template>
  <div v-show="isVisible">
      <div class="popup">
          ...
          <div v-if="type === 'alert'">
            <div class="btnCommon confirm" @click="close()">确定</div>
          </div>
          <div v-else>
            <div class="space-round">
                <div class="btnCommon cancel" @click="cancelEvent()">取消</div>
                <div class="btnCommon confirm" @click="confirmEvent()">确定</div>
            </div>
          </div>
      </div>
...
export default {
  props: {
    ...
    confirm: {
      type: Function,
      default: () => { console.log('请传入确定回调') }
    },
    cancel: {
      type: Function,
      default: () => { console.log('请传入取消回调') }
    }
  },
  methods: {
    close () {
